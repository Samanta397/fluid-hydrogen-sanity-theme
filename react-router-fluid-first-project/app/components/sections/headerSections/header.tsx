...
export function Header({enableCart, enableLogin, enableSearch, navLinks, title}: HeaderProps) {
  return (
    <Container>
        ...
        <nav >
          <ul className={'flex justify-center gap-4'}>
            {navLinks.map((link, index) => (
              <NavLink to={'/'} key={index} className={'active:text-[var(--text-base-color)] active:border-b'}> <li>{link.title}</li> </NavLink>
            ))}
          </ul>
        </nav>
      </header>
